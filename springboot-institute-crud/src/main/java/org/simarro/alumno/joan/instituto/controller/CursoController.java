...
public class CursoController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Created: 'Curso' creado exitosamente", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CursoEdit.class)) }),
            @ApiResponse(responseCode = "400", description = "Bad Request: Errores de validación en los datos proporcionados (errorCode='CURSO_CREATE_VALIDATION')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = BindingResultErrorsResponse.class)) }),
            @ApiResponse(responseCode = "400", description = "Bad Request: Error porque el Id del Curso debe ser nulo (errorCode='CURSO_ID_MISMATCH')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) }),
            @ApiResponse(responseCode = "409", description = "Conflict: Error al intentar crear un 'Curso' (errorCodes: 'FOREIGN_KEY_VIOLATION', 'UNIQUE_CONSTRAINT_VIOLATION', 'DATA_INTEGRITY_VIOLATION')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) })
    })
    @PostMapping
     ...
     * @return Un objeto CursoEdit con los datos del Curso, junto con un código de
     *         estado HTTP 200 (OK).
     *         * @throws DataValidationException Si el formato del id es incorrecto.
     * @throws EntityNotFoundException Si no se encuentra un Curso con el Id
     *                                 especificado.
     */
    @Operation(summary = "Devuelve los datos de un Curso dado su ID.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK: Curso encontrado con éxito", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CursoEdit.class)) }),
            @ApiResponse(responseCode = "400", description = "Bad Request: Error de validación en el ID proporcionado (errorCode='ID_FORMAT_INVALID','DATA_CONVERSATION_ERROR'))", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) }),
            @ApiResponse(responseCode = "404", description = "Not Found: No se encontró el Curso con el ID proporcionado (errorCode='CURSO_NOT_FOUND')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) })
    })
    @GetMapping("/{id}")
     ...
     * @param bindingResult Resultado de las validaciones de los datos
     *                      proporcionados.
     * @return Un objeto CursoEdit con los datos actualizados, junto con un código
     ...
     *                                         objeto CursoEdit.
     * @throws DataValidationException         Si el formato del id es incorrecto.
     * @throws EntityIllegalArgumentException  Si el id especificado no coincide con
     *                                         el del Curso a actualizar.
     * @throws EntityNotFoundException         Si no se encuentra un Curso con el id
     *                                         especificado.
     * @throws DataIntegrityViolationException Si hay errores al almacenar el
     *                                         registro en la BD (clave ajena,
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "OK: Curso actualizado con éxito", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CursoEdit.class)) }),
            @ApiResponse(responseCode = "400", description = "Bad Request: Errores de validación en los datos proporcionados (errorCodes='CURSO_UPDATE_VALIDATION')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = BindingResultErrorsResponse.class)) }),
            @ApiResponse(responseCode = "400", description = "Bad Request: Error de validación en el ID proporcionado (errorCodes='DATA_CONVERSATION_ERROR','ID_FORMAT_INVALID','CURSO_ID_MISMATCH'))", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) }),
            @ApiResponse(responseCode = "404", description = "Not Found: No se encontró el Curso con el ID proporcionado (errorCode='CURSO_NOT_FOUND')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) }),
            @ApiResponse(responseCode = "409", description = "Conflict: Error al intentar actualizar un 'Curso' (errorCodes: 'FOREIGN_KEY_VIOLATION', 'UNIQUE_CONSTRAINT_VIOLATION', 'DATA_INTEGRITY_VIOLATION')", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) })
    })
    @PutMapping("/{id}")
    public ResponseEntity<CursoEdit> update(@PathVariable String id, @Valid @RequestBody CursoEdit cursoEdit,
