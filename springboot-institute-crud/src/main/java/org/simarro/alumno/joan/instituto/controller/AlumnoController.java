...
import org.simarro.alumno.joan.instituto.model.DniString;
...
public class AlumnoController {
         ...
         * @throws HttpMessageNotReadableException Si no coincide el tipo de dato
         *                                         de algun atributo y no se puede
         *                                         parsear.
         * @throws BindingResultException          Si hay errores de validación en
         *                                         el
         *                                         objeto alumnoEdit.
         * @throws EntityAlreadyExistsException    Si ya existe un alumno con el
         *                                         DNI
         *                                         especificado.
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Created: 'Alumno' creado exitosamente", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = AlumnoEdit.class)) }),
                        @ApiResponse(responseCode = "400", description = "Bad Request: Errores de validación en los datos proporcionados (errorCode='STUDENT_CREATE_VALIDATION','DATA_CONVERSATION_ERROR')", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = BindingResultErrorsResponse.class)) }),
                        @ApiResponse(responseCode = "409", description = "Conflict: Error al intentar crear un 'Alumno' (errorCodes: 'STUDENT_ALREADY_EXIST', 'FOREIGN_KEY_VIOLATION', 'UNIQUE_CONSTRAINT_VIOLATION', 'DATA_INTEGRITY_VIOLATION')", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) })
        })
        @PostMapping
        public ResponseEntity<AlumnoEdit> create(@Valid @RequestBody AlumnoEdit alumnoEdit,
        ...
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "400", description = "Bad Request: Errores de validación en los datos proporcionados (errorCode='STUDENT_UPDATE_VALIDATION')", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = BindingResultErrorsResponse.class)) }),
                        @ApiResponse(responseCode = "400", description = "Bad Request: Errores de validación en el DNI proporcionado (errorCodes='DATA_CONVERSATION_ERROR','DNI_FORMAT_INVALID','STUDENT_DNI_MISMATCH')", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = CustomErrorResponse.class)) }),
